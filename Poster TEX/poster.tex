...
\usepackage{float}    % Required for the [H] option in figure environment
...
%\color{black}
\item therefore replicable, transparent, shareable, trustworthy, verifiable and accountable
%\color{uzhockerrot100}
...
\vspace{1cm}
\begin{itemize}
    \item[\ding{228}]  \textbf{SDC} is the traditional approach to protecting outputs against re-identification
        \begin{itemize}
            \item \textbf{Non-perturbation methods} (alter data without changing its actual values)
                \begin{itemize}
...
\vspace{1cm}
            \item \textbf{Perturbation methods} (modify data) %(add noise or alter data so it modifies  data)
                \begin{itemize}
                    \item Noise masking
...
\textbf{Due to today's threats, traditional SDC methods are increasingly inadequate to protect data}, necessitating advanced techniques. 
%It's necessary to use a more modern approach.
